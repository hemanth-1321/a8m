...
from models.schema import UserModel, UserResponse,FastApiResponseWrapper
from api.controllers.userController import signup_user,signin_user
USER_ROUTES=APIRouter()

@USER_ROUTES.post("/signup",response_model=FastApiResponseWrapper)
def create_user(user:UserModel,db:Session=Depends(get_db)):
    response,data=signup_user(user,db)
    return FastApiResponseWrapper(response=response,data=data)


@USER_ROUTES.post("/signin",response_model=FastApiResponseWrapper)
