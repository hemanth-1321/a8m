...
interface SavedNode extends RawNode {
  position_x: number;
  position_y: number;
  type: string;
  provider_type: string;
...
const Page = () => {
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      ...
      const connectionKey = `${params.source}-${
  ...
  const addNode = (provider: Provider) => {
    const newId = uuidv4();
  ...
  const executeWorkflow = async () => {
    ...
    try {
      ...
      toast.error(
        axiosError?.response?.data?.message || "Failed to execute workflow"
      );
    } finally {
      setExecuting(false);
    }
    ...
    const fetchWorkflow = async () => {
      try {
        ...
        const loadNodes: Node<NodeData>[] = (wf.nodes || []).map((n) => {
          ...
          const nestedData = nodeData?.data as
  ...
  const saveWorkflow = async () => {
    ...
    try {
      ...
      const response = await axios.post<{ data: SavedWorkflow }>(
        `${BACKEND_URL}/workflows/${id}`,
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
      ...
      const syncedNodes: Node<NodeData>[] = savedWf.nodes.map(
        (n: SavedNode) => {
          const nodeData = n.data as Record<string, unknown> | undefined;
          const nodeType =
            n.type ||
      ...
      setSaveWorkflowLoading(false);
      const axiosError = error as AxiosError<ApiErrorResponse>;
      console.error("Failed to save workflow:", axiosError.response || error);

      if (axiosError.response?.data?.detail) {
        console.error("Validation errors:", axiosError.response.data.detail);
        toast.error(
          `Validation error: ${
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${
                      workflowEnabled ? "bg-green-500" : "bg-red-500"
                    }`}
                  ></div>
