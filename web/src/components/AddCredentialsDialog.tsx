...
// ✅ JSON types for credentials
type JsonValue = string | number | boolean | JsonObject | JsonValue[];
interface JsonObject {
  [key: string]: JsonValue;
}

// ✅ Subfield definition for repeatable fields
interface SubField {
  key: string;
  placeholder?: string;
}

// ✅ Field definition from provider config
interface ProviderField {
  ...
  repeatable?: boolean;
  fields?: SubField[]; // only if repeatable
}
...
export default function AddCredentialsDialog({ onCredentialsAdded }: Props) {
  ...
  const [credData, setCredData] = useState<JsonObject>({});
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="group relative overflow-hidden bg-gradient-to-r from-slate-900 to-slate-800 hover:from-slate-800 hover:to-slate-700 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border-0 text-white">
          ...
          <span className="font-medium">Add Credentials</span>
        </Button>
      </DialogTrigger>
            ...
            <div className="space-y-4">
              {selectedProviderData?.fields.map((field) => {
                if (
                  ...
                  const currentValues: JsonObject[] =
                    (credData[field.key] as JsonObject[]) || [];
                ...
                return (
                  <div key={field.key} className="space-y-2">
                    ...
                    <Input
                      type="text"
                      value={(credData[field.key] as string) || ""}
                      onChange={(e) =>
                        setCredData({
                          ...credData,
