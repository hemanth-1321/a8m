...
export default function ProviderNode({ id, data, deleteNode }: any) {
  ...
  // Consistent styling for all nodes - darker in light mode
  const getNodeStyles = () => {
    return {
      border: `2px solid ${isDark ? "#374151" : "#6B7280"}`,
      background: isDark
  ...
  const getIconClass = () => {
    return isDark ? "text-gray-300" : "text-gray-300";
  };
    ...
    >
      ...
      <div
        className={`absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none
                    ${isDark ? "shadow-gray-400/20" : "shadow-gray-300/30"} 
                    shadow-lg`}
      />
    </div>
  );
